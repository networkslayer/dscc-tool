        ...
        if isinstance(value, str) and value.startswith("<") and value.endswith(">"):
            ...
            if any(isinstance(v, str) and v.startswith("<") and v.endswith(">") for v in value):
                ...
                if key == "platform":
                    ...
                    #cleaned[key] = prompt(key, suggestion="metadata/screenshots/0.png", validator=files_exist)
                else:
...
def generate_tests(app_path=".", overwrite=False, dry_run=False, noninteractive=False, no_sample=False):
    ...
    print("\n🧪 DSCC Test Generator")
    print("This tool analyzes your detection notebooks and generates dscc-tests YAML and optional sample data.\n")

    print(f"🔍 Scanning {base_path} for notebooks...\n")

    for notebook in base_path.rglob("*.py"):
